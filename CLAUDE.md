...
- `bash scripts/ai-delegate.sh quick "<prompt>"`

### #precise → Codex
- 정확도가 생명인 작업
- 코드 변경, 테스트 작성, 설정 파일 수정
- `bash scripts/ai-delegate.sh precise "<prompt>"`

### #spec → Kiro
- 설계가 필요한 작업
- 신규 기능 요구사항 분석, 아키텍처 설계, 태스크 분해
- `bash scripts/ai-delegate.sh spec "<prompt>"`

### #cross → Gemini + Codex 병렬
- 교차 검증이 필요한 고위험 작업
- 보안 변경, IAM 정책, 네트워크 규칙
- 두 결과를 Master가 비교 판정
- `bash scripts/ai-delegate.sh cross "<prompt>"`

### #critical → Kiro → Codex → Gemini → Master 순차
- 최고 위험 작업 (프로덕션 배포, 장애 대응, 데이터 마이그레이션)
- 각 단계 결과가 다음 단계의 입력
- `bash scripts/ai-delegate.sh critical "<prompt>"`

### #plan → Kiro → Codex → Master
- 스펙 기반 구현 파이프라인
- Kiro가 스펙 생성 → Codex가 스펙 기반 구현 → Master가 적합성 판정
- `bash scripts/ai-delegate.sh plan "<prompt>"`

---

## 자동 승격 규칙

사용자가 모드를 명시하지 않았거나 `#solo`일 때, 아래 조건에 해당하면 자동 승격합니다.

### 승격 (더 신중하게)
```
solo → cross    : 보안 관련 코드 변경 (IAM, SG, 인증, 암호화, 키 관리)
solo → critical : 프로덕션 배포/롤백, 데이터 삭제, 인프라 대규모 변경
solo → spec     : 신규 기능 설계, 아키텍처 리팩토링, 마이크로서비스 분리
quick → cross   : Gemini 결과에서 보안 리스크 감지 시
```
...
2. **타임아웃은 가드레일** — 초과 시 Master가 solo로 fallback
...
# 모드: quick | precise | spec | cross | critical | plan

# 시뮬레이션 (dry run)
bash scripts/simulate.sh <scenario>  # quick | plan | critical | all
