...
| 편 | 유스케이스 | 모드 | 핵심 가치 |
|----|-----------|------|----------|
| **DevOps편** | UC1. IAM 정책 변경 검토 | `@verify` 교차검증 | 보안 실수 원천 차단 |
| | UC2. CloudWatch 로그 대량 분석 | `@scan` 속도우선 | 10만 줄 로그를 45초 만에 |
| | UC3. Terraform 모듈 리팩토링 | `@build` 스펙→구현 | 설계 없는 리팩토링은 재앙 |
| | UC4. CI/CD 파이프라인 장애 대응 | `@mobilize` 풀파이프라인 | 장애 시 4-Agent 총동원 |
| | UC5. ECS 서비스 오토스케일링 튜닝 | `@craft` 정밀분석 | 숫자 하나가 비용을 좌우 |
| **SRE편** | UC6. 프로덕션 P1 장애 대응 | `@mobilize` 풀파이프라인 | 분 단위 의사결정 |
| | UC7. SLO 기반 알림 설정 | `@verify` 교차검증 | 알림 폭풍 vs 누락 사이 |
| | UC8. 비용 이상 탐지 분석 | `@scan` 속도우선 | 과금 폭탄 조기 발견 |
| | UC9. 재해복구(DR) 런북 작성 | `@design` 설계먼저 | 런북은 설계의 산물 |
| | UC10. 보안 감사 대응 자동화 | `@verify`→`@mobilize` 자동승격 | 감사 대응은 교차검증+풀파이프라인 |
| **개발자편** | UC11. 레거시 API → REST 마이그레이션 | `@build` 스펙→구현 | 스펙 없는 마이그레이션은 실패 |
| | UC12. 유닛 테스트 일괄 생성 | `@craft` 정밀분석 | 커버리지 80% 달성 |
| | UC13. PR 코드 리뷰 자동화 | `@verify` 교차검증 | 두 눈보다 네 눈 |
| | UC14. 신규 마이크로서비스 설계 | `@design` 설계먼저 | 설계 먼저, 코드는 나중에 |
| | UC15. 프로덕션 DB 마이그레이션 | `@mobilize` 풀파이프라인 | 데이터는 되돌릴 수 없다 |
...
### Multi-Agent 해법: `@verify` (교차검증)

```
사용자: @verify 이 IAM 정책을 검토해줘
```

**왜 `@verify`인가?** IAM은 보안 관련 코드 변경 → `@ask → @verify` 자동 승격 대상입니다.
...
> ★ **Insight**: `@scan` 모드는 "일단 빨리 파악"이 목적입니다. 만약 Gemini 결과에서
> 보안 리스크가 감지되면 Master가 자동으로 `@verify`로 승격합니다.
...
> ★ **Insight**: `@build` 모드의 핵심은 **Kiro가 "왜 이렇게 분리하는가"를 설계**하고,
...
### Multi-Agent 해법: `@mobilize` (풀파이프라인)

```
사용자: @mobilize GitHub Actions deploy 실패 — "No hosted workload identity pool configured"
```

CI/CD 장애는 배포 차단이므로 `@ask → @mobilize` 자동 승격 대상입니다.
...
> ★ **Insight**: `@mobilize`의 위력은 **각 에이전트가 다른 각도에서 문제를 봅니다**.
...
### Multi-Agent 해법: `@craft` (정밀분석)

```
사용자: @craft ECS api-server 서비스의 오토스케일링 설정 최적화해줘
```
...
> ★ **Insight**: `@craft` 모드는 **숫자가 중요한 작업**에 최적입니다.
...
> ★ **Insight**: P1 장애에서 `@mobilize`가 빛나는 이유는 **병렬 사고**입니다.
...
```
사용자: @verify 결제 서비스의 SLO 기반 알림을 설정해줘. 현재 99.9% SLO 목표.
```
...
> ★ **Insight**: `@verify`에서 Gemini는 "숲"을, Codex는 "나무"를 봅니다.
...
```
사용자: @scan 이번 달 AWS 비용 급증 원인 분석해줘
```
...
### Multi-Agent 해법: `@design` (설계먼저)

```
사용자: @design 현재 프로덕션 아키텍처 기반으로 DR 런북 작성해줘
```
...
> ★ **Insight**: `@design` 모드에서 Kiro는 **"무엇을 해야 하는가"를 설계**합니다.
...
### Multi-Agent 해법: `@verify` → `@mobilize` (자동 승격)

```
사용자: @verify SOC2 감사 체크리스트 기반으로 보안 현황 점검해줘
```

보안 관련 작업이므로 Master가 `@verify → @mobilize` 자동 승격을 판단합니다.

### 예제 출력 (자동 승격 → Mobilize 실행)

```
⚠ 자동 승격: @verify → @mobilize
...
> 사용자는 `@verify`를 요청했지만, Master가 보안 감사의 범위와 위험도를 판단하여
> `@mobilize`로 승격했습니다. 87개 항목을 3-Agent가 분담하여 처리 — 이것이
> "더 신중하게"의 의미입니다.
...
```
사용자: @build legacy SOAP API를 REST로 마이그레이션해줘. 3개 클라이언트 팀 호환 필수.
```
...
2. 인증 전환 레이어 설계 필요 → 후속 `@design` 태스크 생성
3. 통합 테스트: SOAP 클라이언트 → 프록시 → REST → 프록시 → SOAP 응답 E2E
```

> ★ **Insight**: `@build` 모드에서 Master의 핵심 역할은 **갭 분석**입니다.
...
```
사용자: @craft services/ 디렉토리의 테스트 없는 함수들에 유닛 테스트 생성해줘
```
...
> ★ **Insight**: `@craft` 모드가 테스트 생성에 최적인 이유는 **코드를 깊이 이해**하기 때문입니다.
> "결제 실패 시 재고가 복원되는가?" 같은 비즈니스 로직 테스트는
> 코드의 분기문과 예외 처리를 정밀 분석해야만 작성 가능합니다.
> `@scan`으로는 `assert result is not None` 수준에 머뭅니다.
...
```
사용자: @design 모놀리스에서 알림 서비스를 마이크로서비스로 분리 설계해줘
```
...
```
사용자: @mobilize users 테이블에 phone_number 컬럼 추가. 5천만 건, 무중단 필수.
```

데이터 변경이므로 `@ask → @mobilize` 자동 승격 대상입니다.
...
> ★ **Insight**: DB 마이그레이션에서 `@mobilize`가 필수인 이유는 **되돌릴 수 없기 때문**입니다.
