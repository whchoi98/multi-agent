...
- 7가지 모드: solo, quick, precise, spec, cross, critical, plan
...
Claude (Master)
    ...
    ├── #solo ─────→ Master 단독 처리
    │
    ├── #quick ────→ Gemini(Speed) ────→ Master 검증
    │
    ├── #precise ──→ Codex(Precision) ──→ Master 검증
    │
    ├── #spec ─────→ Kiro(Spec) ────────→ Master 검증
    │
    ├── #cross ────→ Gemini + Codex (병렬) ──→ Master 비교 판정
    │
    ├── #critical ─→ Kiro → Codex → Gemini (순차) ──→ Master Go/No-Go
    │
    └── #plan ─────→ Kiro(스펙) → Codex(구현) ──→ Master 갭 분석
```
