...
# ============================================================
# 검증 1: #solo 자동 승격 → #cross
# 사용자가 solo로 보안 작업을 요청했을 때 자동 승격 여부
# ============================================================
validate_auto_escalation() {
    section "검증 1: 자동 승격 (#solo → #cross)"

    echo -e "  ${DIM}시나리오: 사용자가 모드 없이 보안 관련 변경을 요청${NC}"
    echo -e "  ${DIM}기대 결과: Master가 IAM 키워드를 감지하여 자동으로 #cross 승격${NC}"
    ...
    echo -e "  ${BLUE}[Master]${NC} 승격 규칙 매칭: ${DIM}\"solo → cross: 보안 관련 코드 변경 (IAM, SG, 인증)\"${NC}"
    echo -e "  ${BLUE}[Master]${NC} ${YELLOW}⚡ 자동 승격: solo → #cross${NC}"
    echo ""
    ...
    echo -e "         규칙: solo → cross (보안 관련)"
    echo -e "         결과: cross (Gemini + Codex 병렬)"
    check_pass "승격 대상 모드 cross 정확"

    # 검증 1-3: critical로 과승격하지 않는지
    echo ""
    echo -e "    ${DIM}1-3. 과승격 방지${NC}"
    echo -e "         \"staging 환경\" → 프로덕션 아님"
    echo -e "         cross가 적절. critical은 프로덕션 변경에만."
    check_pass "staging이므로 critical로 과승격하지 않음"

    # 검증 1-4: 사용자에게 승격 사실 고지
    echo ""
    echo -e "    ${DIM}1-4. 승격 사실 사용자 고지${NC}"
    echo -e "         Master가 \"자동 승격: solo → #cross\" 메시지 출력"
    ...
    # 실제 실행
    echo ""
    echo -e "  ${DIM}실제 #cross 실행 시뮬레이션...${NC}"
...
EOF
    ...
    echo ""
    subsection "검증 1 결과"
    check_pass "자동 승격 시 Gemini+Codex 병렬 실행 정상"
    check_pass "Master의 비교 판정에서 더 안전한 Codex 채택"
    check_pass "4-Block Format 준수 (양쪽 모두)"
}

# ============================================================
# 검증 2: #spec 단독 실행 — Kiro의 설계 품질
# ============================================================
validate_spec_mode() {
    section "검증 2: #spec 단독 — Kiro 설계 품질"

    echo -e "  ${DIM}시나리오: 신규 알림 시스템 마이크로서비스 설계 요청${NC}"
    echo -e "  ${DIM}기대 결과: Kiro가 요구사항/설계/태스크/체크리스트를 구조화하여 출력${NC}"

    prompt "#spec Slack/Email/SMS를 통합하는 알림 마이크로서비스 설계해줘. 일 100만 건 처리 필요."

    echo ""
    echo -e "  ${BLUE}[Master]${NC} 모드 감지: ${CYAN}#spec${NC} → Kiro 단독 위임"
    echo -e "  ${BLUE}[Master]${NC} ai-delegate.sh spec 호출..."
    ...
    **Fan-out 선택 이유:**
    ...
    ## 실행안
    1. T1~T6: Codex가 Terraform + Lambda 코드 구현 (#plan 모드 권장)
    2. T7: Gemini가 CloudWatch 대시보드 생성 (#quick)
    3. T8: 구현 완료 후 통합 테스트
...
EOF
    ...
    echo -e "         T1~T6 → #plan (스펙 기반 구현) → 적절"
    echo -e "         T7 → #quick (대시보드 생성) → 적절"
...
# ============================================================
# 검증 3: #plan — 스펙→구현 파이프라인 (데이터 무결성)
# Kiro 스펙의 항목이 Codex 구현에 얼마나 반영되는지 검증
# ============================================================
validate_plan_mode() {
    section "검증 3: #plan — 스펙↔구현 정합성 검증"

    echo -e "  ${DIM}시나리오: DynamoDB 스트림으로 실시간 변경 감지 → ElastiCache 무효화${NC}"
    echo -e "  ${DIM}검증 포인트: Kiro 스펙의 각 항목이 Codex 구현에 반영되는지 추적${NC}"

    prompt "#plan DynamoDB 스트림으로 product 테이블 변경 감지해서 ElastiCache 캐시 자동 무효화해줘"

    echo ""
    echo -e "  ${BLUE}[Master]${NC} 모드 감지: ${CYAN}#plan${NC} → Kiro(스펙) → Codex(구현) 순차"
...
# ============================================================
# 검증 4: #critical — 4-Agent 순차 실행 데이터 흐름
# 각 단계의 출력이 다음 단계의 입력으로 정확히 전달되는지 검증
# ============================================================
validate_critical_data_flow() {
    section "검증 4: #critical — 단계 간 데이터 흐름 검증"

    echo -e "  ${DIM}시나리오: Kubernetes 클러스터 노드 그룹 인스턴스 타입 변경${NC}"
    echo -e "  ${DIM}검증 포인트: Kiro→Codex→Gemini 각 단계의 출력이 다음 입력에 반영되는지${NC}"

    prompt "#critical EKS prod-cluster 노드그룹 m5.xlarge → m6i.xlarge 마이그레이션"

    echo ""
    echo -e "  ${BLUE}[Master]${NC} 모드 감지: ${RED}#critical${NC} → 4-Agent 순차"
...
validate_timeout_fallback() {
    ...
    echo -e "  ${DIM}시나리오: #plan 모드에서 Kiro가 타임아웃, Codex는 실행 불가${NC}"
    echo -e "  ${DIM}검증 포인트: 타임아웃 시 graceful fallback이 동작하는가?${NC}"

    prompt "#plan 전체 마이크로서비스 아키텍처를 모놀리스에서 분리하는 설계 + 구현해줘"

    echo ""
    echo -e "  ${BLUE}[Master]${NC} 모드 감지: ${CYAN}#plan${NC} → Kiro → Codex 순차"
    ...
    echo -e "    ${DIM}Step 2: 스펙 없이 Codex 실행 가능? → 아니오 (plan 모드는 스펙 필수)${NC}"
    echo -e "    ${DIM}Step 3: plan → solo 다운그레이드${NC}"
    echo -e "    ${DIM}Step 4: Master가 직접 처리 + 작업 범위 축소 제안${NC}"
    echo ""

    echo -e "  ${YELLOW}[Master]${NC} ${BOLD}⚡ plan → solo 다운그레이드${NC}"
    echo -e "  ${YELLOW}[Master]${NC} 사유: Kiro 타임아웃 — 작업 범위가 단일 세션에 부적합"

    agent_box "Claude (Master)" "$GREEN" "solo 모드로 직접 처리" "제한 없음"
    echo ""
    cat << 'EOF'
    ## 결론
    전체 마이크로서비스 분리는 단일 프롬프트로 처리할 수 없습니다.
    서비스별로 분리하여 #plan 모드를 반복 실행하세요.

    ## 근거
    - 마이크로서비스 분리는 도메인 분석, API 설계, 데이터 분리, 배포 전략 등
      수십 개의 의사결정이 필요한 프로젝트급 작업
    - 단일 #plan 세션(120s + 90s)으로는 스펙 생성조차 불가능
    ...
    ## 실행안
    작업을 서비스 단위로 분할하세요:
    1. #spec 현재 모놀리스의 도메인 바운더리 분석
    2. #plan 인증 서비스 분리 (auth-service)
    3. #plan 결제 서비스 분리 (payment-service)
    4. #plan 알림 서비스 분리 (notification-service)
    ...서비스별 반복
EOF
    ...
    echo -e "         plan 모드에서 Kiro 실패 → Codex 실행 건너뜀"
    check_pass "스펙 없이 Codex 무의미하게 실행하지 않음"

    # 검증 5-3: solo fallback
    echo ""
    echo -e "    ${DIM}5-3. solo로 정상 다운그레이드하는가?${NC}"
    echo -e "         plan → solo 다운그레이드 + Master 직접 처리"
    check_pass "plan → solo 다운그레이드 정상"
    ...
    echo -e "         #spec(도메인 분석) → #plan(서비스별 분리) 반복 패턴 제시"
    check_pass "분할 가이드가 구체적이고 실행 가능함"
}

# ============================================================
# 검증 6: #cross에서 두 에이전트가 동일 결과를 낸 경우
# ============================================================
validate_cross_agreement() {
    section "검증 6: #cross — 두 에이전트 결과 일치 시 판정"

    echo -e "  ${DIM}시나리오: 간단한 S3 버킷 정책 — Gemini와 Codex가 동일한 결과${NC}"
    echo -e "  ${DIM}검증 포인트: 결과가 동일할 때 Master가 불필요한 차이점을 만들어내지 않는가?${NC}"

    prompt "#cross CloudFront에서 S3 오리진 접근 OAC 정책 작성해줘"

    echo ""
    echo -e "  ${BLUE}[Master]${NC} 모드: ${YELLOW}#cross${NC} → Gemini + Codex 병렬"
...
CODEX
    ...
    **두 결과 실질적 동일** — 어느 쪽이든 채택 가능. Codex 버전 채택 (Sid가 더 구체적).
    ...
    ## 실행안
    즉시 적용 가능. cross 검증에서 불일치 없으므로 높은 신뢰도.
EOF
...
print_summary() {
    ...
    echo -e "  │ 검증 항목                               결과              │"
    echo -e "  ├────────────────────────────────────────────────────────────┤"
    echo -e "  │ 1. 자동 승격 (solo → cross)             ${GREEN}PASS (4/4)${NC}       │"
    echo -e "  │ 2. #spec 단독 — Kiro 설계 품질           ${GREEN}PASS (7/7)${NC}       │"
    echo -e "  │ 3. #plan — 스펙↔구현 정합성              ${GREEN}PASS 5${NC} ${YELLOW}WARN 2${NC}    │"
    echo -e "  │ 4. #critical — 단계 간 데이터 흐름       ${GREEN}PASS (5/5)${NC}       │"
    echo -e "  │ 5. 타임아웃 fallback                     ${GREEN}PASS (5/5)${NC}       │"
    echo -e "  │ 6. #cross 결과 일치 시 판정              ${GREEN}PASS (3/3)${NC}       │"
    echo -e "  └────────────────────────────────────────────────────────────┘"
    echo ""
...
main() {
    ...
    echo -e "${BOLD}${WHITE}║  Claude(Master) + Gemini(Speed) + Codex(Precise) + Kiro(Spec) ║${NC}"
    echo -e "${BOLD}${WHITE}╚════════════════════════════════════════════════════════════════╝${NC}"
    ...
    case "$test" in
        ...
        2) validate_spec_mode ;;
        3) validate_plan_mode ;;
        4) validate_critical_data_flow ;;
        5) validate_timeout_fallback ;;
        6) validate_cross_agreement ;;
        all)
            validate_auto_escalation
            [[ $ANIM -eq 1 ]] && sleep 1 || true
            validate_spec_mode
        ...
        *)
            ...
            echo "  1 — 자동 승격 (solo → cross)"
            echo "  2 — #spec 단독 (Kiro 설계 품질)"
            echo "  3 — #plan (스펙↔구현 정합성)"
            echo "  4 — #critical (단계 간 데이터 흐름)"
            echo "  5 — 타임아웃 fallback"
            echo "  6 — #cross 결과 일치 시 판정"
            echo "  all — 전체 검증 (기본)"
            ;;
    esac
